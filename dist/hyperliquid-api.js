...
exports.EnhancedHypeTradeMonitor = void 0;
class EnhancedHypeTradeMonitor {
    ...
    async getAllTokenPrices() {
        if (this.priceCache.has('allMids') && Date.now() - this.priceCache.get('allMids').timestamp < 30000) {
            return this.priceCache.get('allMids').data;
        }
        const data = await this.makeApiCall({ type: 'allMids' });
        if (data)
            this.priceCache.set('allMids', { data, timestamp: Date.now() });
        return data;
    }
    async getCurrentPrice() {
        const prices = await this.getAllTokenPrices();
        return (prices === null || prices === void 0 ? void 0 : prices.HYPE) ? parseFloat(prices.HYPE) : null;
    }
    async getWalletPortfolio(walletAddress) {
        const cacheKey = `portfolio_${walletAddress}`;
        if (this.walletCache.has(cacheKey) && Date.now() - this.walletCache.get(cacheKey).timestamp < 300000) {
    ...
    async processPortfolioData(ch, fills, wallet) {
        var _a, _b, _c;
        const prices = await this.getAllTokenPrices();
        if (!prices)
            return null;
        const portfolio = { walletAddress: wallet, totalValue: 0, positions: [], recentActivity: [] };
        if (ch === null || ch === void 0 ? void 0 : ch.assetPositions) {
            for (const p of ch.assetPositions) {
                const coin = (_a = p.position) === null || _a === void 0 ? void 0 : _a.coin;
                const size = parseFloat(((_b = p.position) === null || _b === void 0 ? void 0 : _b.szi) || '0');
                const entryPx = parseFloat(((_c = p.position) === null || _c === void 0 ? void 0 : _c.entryPx) || '0');
                if (coin && prices[coin] && Math.abs(size) > 0) {
                    const currentPx = parseFloat(prices[coin]);
        ...
        if (Array.isArray(fills)) {
            const recent = fills.filter((f) => Date.now() - (f.time || 0) < 7 * 24 * 60 * 60 * 1000);
            const net = {};
            ...
            portfolio.recentActivity = recent.slice(0, 5).map((t) => ({
    ...
    getTop3Holdings(p) {
        var _a;
        return ((_a = p === null || p === void 0 ? void 0 : p.positions) === null || _a === void 0 ? void 0 : _a.filter((pos) => pos.value > 0).sort((a, b) => b.value - a.value).slice(0, 3).map((pos) => ({
    ...
    async detectWalletFromTrade(trade) {
        ...
        for (const addr of walletList) {
            ...
            const hypeRecent = fills === null || fills === void 0 ? void 0 : fills.filter((f) => f.coin === 'HYPE' && Date.now() - (f.time || 0) < 3600000);
            if ((hypeRecent === null || hypeRecent === void 0 ? void 0 : hypeRecent.length) > 0)
                return addr;
        }
        return fallback;
    }
    ...
    detectLargeTradesFromCandles(candles) {
        return (candles || []).flatMap((c) => {
            const v = parseFloat(c.v);
            const avg = (parseFloat(c.o) + parseFloat(c.c) + parseFloat(c.h) + parseFloat(c.l)) / 4;
            const dollarVol = v * avg;
            if (dollarVol > this.threshold) {
                const priceMove = ((parseFloat(c.c) - parseFloat(c.o)) / parseFloat(c.o)) * 100;
                return [{
                        timestamp: c.t, side: parseFloat(c.c) > parseFloat(c.o) ? 'BUY' : 'SELL',
    ...
    calculateConfidence(v, dollarVol, change) {
        let conf = 0;
        if (dollarVol > 500000)
            conf += 40;
        else if (dollarVol > 250000)
            conf += 30;
        else if (dollarVol > 100000)
            conf += 20;
        if (change > 5)
            conf += 30;
        else if (change > 2)
    ...
    formatEnhancedAlert(trade, price, wallet, portfolio) {
        ...
        msg += `💰 Value: ${trade.dollarVolume.toLocaleString()}\n📊 Side: ${trade.side}\n📈 Price Impact: ${trade.priceChange.toFixed(2)}%\n🎯 Current Price: ${price}\n⏰ Time: ${ts}\n🔍 Confidence: ${trade.confidence}%`;
        if ((portfolio === null || portfolio === void 0 ? void 0 : portfolio.totalValue) > 0) {
            msg += `\n🏦 WALLET: ${wallet.slice(0, 8)}...${wallet.slice(-6)}\n💎 Portfolio: ${portfolio.totalValue.toLocaleString()}\n🏆 Top Holdings:`;
            top.forEach((h) => msg += `\n🥇 ${h.coin}: ${h.value.toLocaleString()} (${h.percentage.toFixed(1)}%)`);
        }
        else {
    ...
    async monitorTrades() {
        ...
        for (const trade of trades) {
            ...
            if (!this.processedTrades.has(id)) {
                ...
                console.log(this.formatEnhancedAlert(trade, price, wallet, portfolio));
                this.processedTrades.add(id);
            }
        }
        if (trades.length === 0)
            console.log(`✅ ${new Date().toLocaleTimeString()} - No large trades.`);
    }
    async start() {
        if (this.isRunning)
            return;
...
Wallet: ${wallet}`);
        ...
        top.forEach((h, i) => console.log(`${i + 1}. ${h.coin} - ${h.value.toLocaleString()}`));
    }
    async findBigWhaleTrades(coin = 'HYPE', valueThreshold = 100000) {
        console.log(`
Analyzing known whales for trades > ${valueThreshold.toLocaleString()} in ${coin}...`);
        ...
        for (const whaleAddress of this.knownWhales) {
            ...
            if (!userFills)
                continue;
            const hypeFills = userFills.filter((f) => f.coin === coin);
            const bigTrades = hypeFills.filter((f) => (parseFloat(f.sz) * parseFloat(f.px)) > valueThreshold);
...
exports.EnhancedHypeTradeMonitor = EnhancedHypeTradeMonitor;
async function main() {
