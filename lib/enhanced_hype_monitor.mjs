...
class EnhancedHypeTradeMonitor {
  ...
  initWebSocket() {
    ...
    this.ws.on('open', () => {
      ...
      this.ws.send(JSON.stringify({ method: 'subscribe', subscription: { type: 'trades', coin: 'HYPE' } }));
    });
  ...
  async handleTrades(trades) {
    for (const trade of trades) {
      ...
      if (trade.side === 'B' && value >= this.threshold) {
        const largeBuy = {
          ...
          value: value,
          wallet: 'N/A', // Not available in public trades
          whaleName: 'Large Buyer',
          orderId: trade.tid,
          confidence: 80,
        };
        
        const currentPrice = await this.getCurrentPrice();
        const telegramAlert = this.formatTelegramAlert(largeBuy, currentPrice, this.technicalData);
        console.log(telegramAlert);
        await this.sendTelegramAlert(telegramAlert, { urgent: true });
